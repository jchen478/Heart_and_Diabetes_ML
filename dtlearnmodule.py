            ...
            for i in range(len(uniqueList)):
                ...
                if i != 0:
                    ...
                    if val != pval:
                        ...
                        Hcond = dfLeft.shape[0]/data.shape[0]*Hleft+dfRight.shape[0]/data.shape[0]*Hright
                        infoGain = H - Hcond
...
def Entropy(data):
    ...
    if nNeg == 0:
        return 0.0
    H = - nPos/nData*math.log(nPos/nData,2)\
        -nNeg/nData*math.log(nNeg/nData,2)
    return H
...
#%% Main Program   
def DTLearn(m,trainFilename,testFilename,pfrac):

    # read input, preprocessing
    dataset, meta = arff.loadarff(trainFilename)
    data = pd.DataFrame(dataset)
    data = data.sample(frac=pfrac)
    ...
    return np.array([nCor, len(dataT)])